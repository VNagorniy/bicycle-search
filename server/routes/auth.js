...
// });
// /api/auth/sign_up
router.post('/sign_up', [check('email', 'Некорректный email').isEmail(), check('password', 'Минимальная длина пароля 8 символов').isLength({ min: 8 })], async (req, res) => {
  try {
...
// /api/auth/sign_in
// router.post('/sign_in', [check('email', 'Некорректный email').normalizeEmail().isEmail(), check('password', 'Введите пароль').exists()], async (req, res) => {
//   try {
//     const errors = validationResult(req);
//     if (!errors.isEmpty()) {
//       return res.status(400).json({
...
//     }
//     const token = jwt.sign({ id: user.clientId }, config.get('jswSecret'), { expiresIn: '7d' });
